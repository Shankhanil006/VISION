...
model_0x = load_model('/media/ece/DATA/Shankhanil/VQA/tmp/contrastive_models/ablation/FvsOF_flow_cmc_0x_3000.h5', custom_objects={'loss': loss_fn})
...
#                    frame_diff = frame11.astype(np.int16) - frame12.astype(np.int16)    
                    flow = optical_flow.calc(cv2.resize(frame11,None, fx=flow_scale,fy=flow_scale),  cv2.resize(frame12,None, fx=flow_scale,fy=flow_scale), None)
                    flow = cv2.resize(flow, None, fx= 1/flow_scale, fy = 1/flow_scale)*(1/flow_scale)
                    
                    framediff_0x, sigma = flow_based_patch_generator(flow, frame11, 0.85).patch_generator()
...
savemat(output_dir + 'cmc_FvsOF_flow_ref_feat_0x_3000.mat',mdic)
