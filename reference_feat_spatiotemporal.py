...
from flow_based_patch_generator import *
#from spatial_base_patch_generator import *
...
output_dir = '/media/ece/DATA/Shankhanil/VQA/contrastive_features/test/'

loss_fn = contrastive_Loss_func(0)
#base_model= get_model()
#model = multi_gpu_model(base_model, gpus=2)

model_0x = load_model('/media/ece/DATA/Shankhanil/VQA/tmp/contrastive_models/test/synthetic_FvsFD_diff_contrastive_0x_10000.h5', custom_objects={'loss': loss_fn})
#model_1x = load_model('/media/ece/DATA/Shankhanil/VQA/tmp/contrastive_models/acm/flow_temporal_contrastive_1x_1000.h5', custom_objects={'loss': loss_fn})
#model_2x = load_model('/media/ece/DATA/Shankhanil/VQA/tmp/contrastive_models/acm/flow_temporal_contrastive_2x_1000.h5', custom_objects={'loss': loss_fn})
#model_3x = load_model('/media/ece/DATA/Shankhanil/VQA/tmp/contrastive_models/acm/flow_temporal_contrastive_3x_5000.h5', custom_objects={'loss': loss_fn})
...
#    for ref_id in range(databases[train_db]['ref']):
    ...
    for name in filenames:
            ...
            for j in range(0, len(get_video)-1, 2):
                    ...
                    framediff_0x, sigma = spatiotemporal_based_patch_generator(frame_diff, frame11, 0.85).patch_generator()
#                    framediff_1x,framediff_2x,framediff_3x = [], [], []
#                    for p in range(len(framediff_0x)):
#                        tmp_list = pyramid(framediff_0x[p],scale)
#                        framediff_1x.append(tmp_list[0])
#                        framediff_2x.append(tmp_list[1])
#                        framediff_3x.append(tmp_list[2])                        
                                                
                        
                    framediff_0x = np.expand_dims(framediff_0x, 1) 
#                    framediff_1x = np.expand_dims(framediff_1x, 1) 
#                    framediff_2x = np.expand_dims(framediff_2x, 1) 
#                    framediff_3x = np.expand_dims(framediff_3x, 1) 
                    
                    tmp_0x = (model_0x.predict([framediff_0x]))#.squeeze(0)
#                    tmp_1x = (model_1x.predict([framediff_1x]))#.squeeze(0)
#                    tmp_2x = (model_2x.predict([framediff_2x]))#.squeeze(0)
#                    tmp_3x = (model_3x.predict([framediff_3x]))#.squeeze(0)
...
savemat(output_dir + 'synthetic_FvsFD_diff_ref_feat_0x_10000.mat',mdic)
